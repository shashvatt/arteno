...
const testimonial = {
  text: "I went from a vague idea to a full product plan in under 2 minutes. This is the tool I wished existed when I started building.",
  name: "Arjun M.",
  role: "Indie Hacker",
};
...
export default function SignUpPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: isMobile ? "column" : "row", fontFamily: "var(--font)", background: "var(--bg)" }}>
      ...
      <div style={{
        ...(isMobile
          ? { width: "100%", padding: "28px 24px 32px", minHeight: 200 }
          : { width: 400, flexShrink: 0, padding: "44px 44px 40px", minHeight: "100vh" }
        ),
        ...
        <div style={{ position: "relative", zIndex: 2, flex: isMobile ? "unset" : 1, display: "flex", flexDirection: "column", justifyContent: "center", ...(isMobile ? {} : { paddingTop: 40, paddingBottom: 40 }) }}>
          <h2 style={{ fontSize: isMobile ? 21 : 27, fontWeight: 700, letterSpacing: "-0.6px", lineHeight: 1.2, color: "#fff", marginBottom: 10, animation: "fadeup 0.5s 0.1s cubic-bezier(0.16,1,0.3,1) both" }}>
            Your product plan,<br />ready in seconds.
          </h2>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.48)", lineHeight: 1.7, marginBottom: isMobile ? 16 : 28, animation: "fadeup 0.5s 0.17s cubic-bezier(0.16,1,0.3,1) both" }}>
            Join builders using Arteno to plan, validate, and launch faster.
          </p>
          <div style={{ padding: "16px 18px", background: "rgba(255,255,255,0.06)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)", animation: "fadeup 0.5s 0.24s cubic-bezier(0.16,1,0.3,1) both" }}>
            <p style={{ fontSize: isMobile ? 12 : 13, color: "rgba(255,255,255,0.72)", lineHeight: 1.7, marginBottom: 12, fontStyle: "italic" }}>
              "{testimonial.text}"
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                {testimonial.name[0]}
              </div>
              <div>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: "#fff" }}>{testimonial.name}</div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.38)" }}>{testimonial.role}</div>
              </div>
            </div>
          </div>
        </div>
        {!isMobile && (
          <div style={{ position: "relative", zIndex: 2, fontSize: 11.5, color: "rgba(255,255,255,0.3)" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "40px 20px 48px" : "40px 32px", overflowY: "auto" }}>
        <div style={{ width: "100%", maxWidth: isMobile ? "100%" : 360, animation: "fadeup 0.5s 0.08s cubic-bezier(0.16,1,0.3,1) both" }}>
          <h1 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, letterSpacing: "-0.4px", marginBottom: 4, color: "var(--text)" }}>
            Create account
          </h1>
          ...
          {success ? (
            <div style={{ fontSize: 13, color: "#22c55e", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 8, padding: "14px 16px", textAlign: "center", lineHeight: 1.6 }}>
              {success}
            </div>
          ) : (
            <form onSubmit={handleEmailSignUp} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              ...
              {error && (
                <div style={{ fontSize: 12.5, color: "#ef4444", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "8px 12px", marginBottom: 12 }}>
                  {error}
                </div>
              )}
          ...
          {isMobile && (
            <div style={{ textAlign: "center", marginTop: 28, fontSize: 12, color: "var(--text-4)" }}>
              {new Date().getFullYear()} Arteno
            </div>
          )}
        </div>
      </div>

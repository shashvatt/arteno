...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const contextBlock = buildContextBlock(ctx, "marketing");
    ...
    const { data: saved, error } = await supabase
      .from("agent_outputs")
      .insert({
        user_id: userId, agent_type: "marketing",
    ...
    await saveStartupContext(userId, {
      positioning:        data.marketingStrategy?.positioning,
      brand_voice:        data.marketingStrategy?.brandVoice,
      primary_keywords:   data.seoStrategy?.primaryKeywords,
      marketing_snapshot: data,
    });
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .eq("user_id", userId).eq("agent_type", "marketing")
